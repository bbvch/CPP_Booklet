...
public:
  template <std::size_t N> decltype(auto) get() const {
    ...
    // note the changing of the type from std::string to const char* for the 2nd
    // parameter of returning a reference to the vector
    if constexpr (N == 0)
      return x;
...
template <std::size_t N> struct tuple_element<N, Bindable> {
