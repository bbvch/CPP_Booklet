...
int main(int, char**)
{
    ...
    const auto capture_all_by_value = [=] () {}; // captures all variables in scope by value (square, r, x,y,z)
    const auto capture_all_by_ref= [&] () {}; // alle Variablen by reference
    const auto capture_all_by_value_except = [=, &x] () {}; // alle by value ausser x by ref
    const auto captire_all_by_ref_except = [&, x] () {}; // Capture all by reference except x
    ...
    std::for_each(v.begin(), v.end(), [&sum] (int i)
    {sum+=i;});

    std::transform(a1.begin(), a1.end(), a2.begin(),
    [](int x) -> int { int y = x % 5; return x * y; } );
}
